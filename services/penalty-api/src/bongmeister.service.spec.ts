...
describe('BongmeisterService', () => {
  ...
  beforeEach(async () => {
    ...
    prisma.add.findUnique.mockResolvedValue(pendingPenalty);
  ...
  describe('moderate', () => {
    it('approves a pending penalty', async () => {
      ...
      await service.moderate(
        pendingPenalty.id,
        { action: PenaltyAction.APPROVE },
        'Bearer token',
      );

      expect(prisma.add.update).toHaveBeenCalledWith({
        where: { id: pendingPenalty.id },
        data: {
          status: approveStatus.APPROVED,
    ...
    it('edits and approves a pending penalty', async () => {
      prisma.add.update.mockResolvedValue({
        ...pendingPenalty,
        amount: 4,
        status: approveStatus.APPROVED,
    ...
    it('rejects a pending penalty', async () => {
      prisma.add.update.mockResolvedValue({
        ...pendingPenalty,
    ...
    it('does not moderate an already handled penalty', async () => {
  ...
  describe('moderateRedeem', () => {
    ...
    it('throws ConflictException when increasing the amount exceeds available balance', async () => {
      ...
      await expect(
        service.moderateRedeem(
          pendingRedemption.id,
          { action: PenaltyAction.APPROVE, amount: 25 },
          'Bearer token',
        ),
      ).rejects.toBeInstanceOf(ConflictException);
      expect(prisma.redemption.updateMany).not.toHaveBeenCalled();
    ...
    it('allows decreasing the amount even if available balance is tight', async () => {
      ...
      await service.moderateRedeem(
        pendingRedemption.id,
        { action: PenaltyAction.APPROVE, amount: 2 }, // additionalAmount = -3
        'Bearer token',
      );
    ...
    it('does not run the balance-sufficiency check when rejecting, regardless of amount', async () => {
      ...
      await expect(
        service.moderateRedeem(
          pendingRedemption.id,
          { action: PenaltyAction.REJECT, amount: 100 },
          'Bearer token',
        ),
